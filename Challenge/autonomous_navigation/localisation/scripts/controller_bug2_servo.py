...
class Bug2Controller:
    def __init__(self):
        ...
        self.goal.pose.position.x = 2.3
        self.goal.pose.position.y = 0.1
    ...
    def return_home(self):
        self.goal_distance = math.sqrt((self.goal.pose.position.x - self.current_pose.pose.position.x)**2 + (self.goal.pose.position.y - self.current_pose.pose.position.y)**2)
        print("goal distance: ", self.goal_distance)
        self.deltax = self.goal.pose.position.x - self.current_pose.pose.position.x
        self.deltay = self.goal.pose.position.y - self.current_pose.pose.position.y

        if self.deltax < 0.05 and self.deltay < 0.05 and self.base_flag ==True:  # Stop if the goal $
            self.current_state = StateMachine.STOP 
       ...
       # self.goal_ack_pub.publish(self.goal_changer[0])
        ...
        while True:
           ...
           # goal_distance = math.sqrt((self.goal.pose.position.x - self.current_pose.pose.position.x)**2 + (self.goal.pose.position.y - self.current_pose.pose.position.y)**2)
            ...
            print("base:",self.base_flag,"cube:",self.cube)
            self.return_home()
            #if self.base_flag == True and self.cube == False:
                #if deltax < 0.1 and deltay < 0.1:  # Stop if the goal $
                 #   self.current_state = StateMachine.STOP 
                    #self.goal_changer[newGoaliD]
                  #  print("Found Goal!")
