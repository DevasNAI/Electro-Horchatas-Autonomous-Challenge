...
class Bug2Controller:
    ...
    def move_to_goal(self):
	    ...
	    if len(self.arucos) > 0:
	    	for aruco in self.arucos.arucos_detected:
		    print("Arucos Detected")
                    if aruco.id == 6 and distance_to_goal <= 0.15:
                    	print("Cube founded")
                    	self.aruco_x = aruco.position.x
                    	self.aruco_y = aruco.position.y
    ...
    def visual_servo(self):
        ...
        if self.aruco_x < 8:
            ...
            if error==0:
                speed = 0
                val = 115
		self.fb_pub.publish("Cubo_obtained")
            else:
                val = 0

        self.cmd_vel.linear.x = line
        self.cmd_vel.angular.z = speed

        self.cmd_vel_pub.publish(self.cmd_vel)
        self.servo_pub.publish(val)
